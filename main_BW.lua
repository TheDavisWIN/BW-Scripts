...
local gymLeaderIDs = {11, 12, 13, 21, 22, 23, 24 ,25, 131, 132, 133}

local partyData = {}
local tempPartyData = {}
local battlePartyData = {}
local tempBattlePartyData = {}
local moveOptions = {}
local battleMenuOptions = {"MAIN", "FIGHT", "BAG", "POKEMON", "RUN"}
local moveMenuOptions = {0, 32, 64, 96}
local moveFile = "C:\\Users\\davis\\BW-Scripts\\moves.txt"
local currentOpponent = None
local previousOpponent = None
...
function setPredictedDateTime()
 ...
 if dateTime["second"] + 5 >= 60 then
  nextSeconds = (dateTime["second"] + 5) - 60
  nextMinutes = dateTime["minute"] + 1
 else
  nextSeconds = dateTime["second"] + 5
...
function buildSeedFromDelay(delay, predictSeedFlag)
 ...
 local macAddr = 0x123456  -- DeSmuMe MAC addres
...
function getInitialSeedInfoInput()
 ...
 gui.text(2, 182, showCGearSeedInfoText and "7 - Hide C-Gear info" or "8 - Show C-Gear info")
end

function showCGearSeedInfo(delay)
 if showCGearSeedInfoText then
  gui.box(1, 67, 164, 152, "#0000007F", "#0000007F")
  gui.text(2, 68, string.format("Next Initial Seed: %08X", buildSeedFromDelay(delay + 308, true)))
  gui.text(2, 79, string.format("Next Delay: %d", delay + 308))
  gui.text(2, 90, string.format("Delay: %d", delay))
  gui.text(2, 101, string.format("C-Gear Seed: %08X", cgearSeed))
  gui.text(2, 112, string.format("Hit Delay: %d", hitDelay))
  gui.text(2, 123, string.format("Hit Date/Hour:\n%s", hitDate))
 end
end
...
-- Checks if the player is in battle
function inBattle()
  activePokemon = memory.readword(0x0226D6B0)
  if (activePokemon ~= 0 and (activePokemon > 0 and activePokemon < 650)) then
    return true
  else
    return false
  end
...
-- Prints value of inBattle()
function printInBattle()
  local key = input.get()
...
-- Converts pokemonPID to 32-bit integer
function readU32LE(address)
  local b1 = memory.readbyte(address)
  local b2 = memory.readbyte(address + 1)
  local b3 = memory.readbyte(address + 2)
  local b4 = memory.readbyte(address + 3)

  return b1 + (b2 * 0x100) + (b3 * 0x10000) + (b4 * 0x1000000)
end

function compareText(a, b)
  fileA = io.open("battle_menu.txt", "r")
  contentA = fileA:read("*all")
end

-- Checks if two tables contain the same keys and values
function compare(a, b)
  if a == b then return true end
  if type(a) ~= "table" or type(b) ~= "table" then return false end
  for key, value in pairs(a) do
    if type(value) == "table" then
      if not compare(value, b[key]) then return false end
...
function updateParty()
  local update = false
  for i = 0, 5 do
    local partyStatsSelectedSlotIndex = read8Bit(partyStatsSelectedSlotIndexAddr) + i
    local pokemonPartyStatsAddr = partyAddr + (partyStatsSelectedSlotIndex * 0xDC)
    printData(pokemonPartyStatsAddr, i)
    if not compare(tempPartyData[i], partyData[i]) then
      update = true
      partyData[i] = tempPartyData[i]
      --print("Changed Party Member ", i)
      --print(partyData[i])
    end
  end
  if update then
...
-- Updates the party in battle
function battleParty()
  local activePokemon = 0
  local activePID = 0
  local firstSlot = memory.readword(0x0226D6B0)
  local optionA = memory.readword(0x022A7E0C)
  if (inBattle() and not optionA == firstSlot and optionA > 0 and optionA < 650) then
    activePokemon = optionA
    activePID = readU32LE(0x022A7E14)
    activePID = string.format("%08X", activePID)
  elseif (inBattle()) then
    activePokemon = firstSlot
    activePID = readU32LE(0x0226A234)
    activePID = string.format("%08X", activePID)
  end
  if (not inBattle() or activePokemon == 0) then
    for i = 0, 5 do
      if not compare(battlePartyData[i], partyData[i]) then
        battlePartyData[i] = partyData[i]
        tempBattlePartyData[i] = battlePartyData[i]
      end
    end
  elseif (inBattle() or (activePokemon > 0 and activePokemon < 650)) then
    for i = 1, 5 do
      if (battlePartyData[i].pid == activePID) then
        print("Switching party member", battlePartyData[0].name, ":", battlePartyData[0].pid, "with")
        print("party member", tempBattlePartyData[i].name, ":", tempBattlePartyData[i].pid)
        tempBattlePartyData[0], tempBattlePartyData[i] = tempBattlePartyData[i], tempBattlePartyData[0]
      end
    end
  end
  for i = 0, 5 do
    if (not compare(battlePartyData[i], tempBattlePartyData[i])) then
      battlePartyData[i] = tempBattlePartyData[i]
    end
  end
end

-- Prints the full details of the party
function printTeam()
  local key = input.get()
  if key["0"] or key["numpad0"] then
    print("Party Data:")
    for i = 0, 5 do
      if (partyData[i].name ~= nil) then
        print(partyData[i])
      end
    end
  end
...
-- Prints the full details of a party member at a given partyIndex
function printData(pidAddr, partyIndex)
  local name
  local pokemonPID = read32Bit(pidAddr)
  pid = string.format("%08X", pokemonPID)
  local checksum = read16Bit(pidAddr + 0x6)
  local orderIndex = (rshift(band(pokemonPID, 0x3E000), 0xD) % 24) + 1
  local move = {}
  ...
  prng = LCRNG(prng, 0x41C64E6D, 0x6073)
  local speciesDexIndex = bxor(read16Bit(pidAddr + growthOffset + 0x8), rshift(prng, 16))
  if (speciesDexIndex > 0 and speciesDexIndex < 650) then
    --print("Party Member Index:", partyIndex)
    --print("Species Name:", speciesNamesList[speciesDexIndex])
    name = speciesNamesList[speciesDexIndex]
    --print("Species Index:", speciesDexIndex)
  end
  ...
  prng = LCRNG(prng, 0x41C64E6D, 0x6073)
  move[1] = bxor(read16Bit(pidAddr + attacksOffset + 0x8), rshift(prng, 16))
  if (move[1] > 1 and speciesDexIndex > 0 and speciesDexIndex < 650) then
    --print(move[1])
    --print("Move 1:", moveNamesList[move[1] + 1])
  end
  prng = LCRNG(prng, 0x41C64E6D, 0x6073)
  move[2] = bxor(read16Bit(pidAddr + attacksOffset + 0xA), rshift(prng, 16))
  if (move[2] > 1 and speciesDexIndex > 0 and speciesDexIndex < 650) then
    --print(move[2])
    --print("Move 2:", moveNamesList[move[2] + 1])
  end
  prng = LCRNG(prng, 0x41C64E6D, 0x6073)
  move[3] = bxor(read16Bit(pidAddr + attacksOffset + 0xC), rshift(prng, 16))
  if (move[3] > 1 and speciesDexIndex > 0 and speciesDexIndex < 650) then
    --print(move[3])
    --print("Move 3:", moveNamesList[move[3] + 1])
  end
  prng = LCRNG(prng, 0x41C64E6D, 0x6073)
  move[4] = bxor(read16Bit(pidAddr + attacksOffset + 0xE), rshift(prng, 16))
  if (move[4] > 1 and speciesDexIndex > 0 and speciesDexIndex < 650) then
    --print(move[4])
    --print("Move 4:", moveNamesList[move[4] + 1])
  end
  tempPartyData[partyIndex] = {name = name, pokedex = speciesDexIndex, position = partyIndex, pid = pid, moves = move, pidAddr = pidAddr, pokemonPID = pokemonPID}
end

-- Writes Move IDs of each of your Active Pokemon's moves to 'moves.txt'
function writeMoveOptions(moveOptions)
  local file = io.open("moves.txt", "w")
  if not file then
    print("File not found: moves.txt")
    return
  end
  for i, move in ipairs(moveOptions) do
    file:write(move .. "\n")
  end
  file:close()
end
...
-- Writes Move IDs of each of your Active Pokemon's moves to 'moves.txt'
function writeTeamMoveOptions(pokedex, moveOptions)
  local file = io.open("team_moves.txt", "a")
  if not file then
    print("File not found: team_moves.txt")
    return
  end
  file:write(pokedex .. "\n")
  for i, move in ipairs(moveOptions) do
...
-- Clears Move IDs from 'move.txt'
function clearTeamMoveOptions()
  local file = io.open("team_moves.txt", "w")
  if file then
    file:close()
  end
end

-- Writes menu state, position, and move selection to 'battle_menu.txt'
function writeBattleMenuOptions(active, opponent, battleMenuOption, moveMenuOption, moveOptions)
  local file = io.open("battle_menu.txt", "w")
  if not file then
    print("File not found: battle_menu.txt")
    return
  end
  file:write(active .. "\n")
  file:write(opponent .. "\n")
  file:write(currentBattleMenuOption .. "\n")
  file:write(currentMoveMenuOption .. "\n")
  file:write(tostring(moveOptions) .. "\n")
  file:close()
end

function saveBattleMenuOptions()
  local file = io.open("battle_menu.txt", "r")
  saved_battle_menu = file:read("*all")
end

-- Clears 'battle_menu.txt'
function clearBattleMenuOptions()
  local file = io.open("battle_menu.txt", "w")
  if file then
    file:close()
  end
...
-- Checks if a value is in a predefined list
...
-- Determines which move selection memory to read from (Wild, Trainer, Gym Leader)
function moveSelectionMemory()
  if (inBattle()) then
    local moveSelectionMemory = 0
    local trainerID = memory.readbyte(0x022697BE)
    if (trainerID == 0) then
      moveSelectionMemory = memory.readbyte(0x022D6B14)
      --print("Wild Pokemon")
    elseif (isInList(trainerID, gymLeaderIDs)) then
      moveSelectionMemory = memory.readbyte(0x022D7A08)
      --print("Gym Leader")
    else
      moveSelectionMemory = memory.readbyte(0x022D81A4)
      --print("Trainer")
    end
    return moveSelectionMemory
  end
end

-- Finds which memory to use to identify opponent
function opponentMemory()
  if (inBattle()) then
    local active = battlePartyData[0].pokedex
    local optionA = memory.readword(0x022A7E68)
    local optionB = memory.readword(0x022A7E0C)
    local opponent
    if (optionA == 0 or optionA == active) then
      opponent = optionB
      --print(opponent)
      return opponent
    elseif (optionB == 0 or optionB == active) then
      opponent = optionA
      --print(opponent)
      return opponent
    else
      opponent = 0
      --print("Opponent not on the field")
      return opponent
    end
  end
end

-- Main method for battle menu
function moveSelection()
  local active = battlePartyData[0].pokedex
  if (inBattle() and active > 0 and active < 650) then
    local moveSelectionMemory = moveSelectionMemory()
    if (not compare(moveOptions, battlePartyData[0].moves)) then
      moveOptions = battlePartyData[0].moves
      writeMoveOptions(moveOptions)
    end
    local opponent = opponentMemory()
    if (opponent > 0 and opponent < 650 and opponent ~= previousOpponent) then
      clearTeamMoveOptions()
      for i = 0, 5 do
        local member = battlePartyData[i]
        if (member.pokedex > 0 and member.pokedex < 650) then
          writeTeamMoveOptions(member.pokedex, member.moves)
        end
      end
      previousOpponent = opponent
      print(previousOpponent)
    end
    currentBattleMenuOption = memory.readword(0x02296288)
    currentMoveMenuOption = moveSelectionMemory / 32
    if (currentBattleMenuOption == 0 or currentBattleMenuOption == 2 or currentBattleMenuOption == 3) then
      writeBattleMenuOptions(active, opponent, currentBattleMenuOption, currentMoveMenuOption, 0)
    elseif (currentBattleMenuOption == 1) then
      writeBattleMenuOptions(active, opponent, currentBattleMenuOption, currentMoveMenuOption, moveOptions[currentMoveMenuOption + 1])
    end
  end
end
...
function learningMove()
  if (inBattle()) then
    newMove = memory.readword(0x021F3B64)
    if (newMove > 0 and newMove < 650) then
      --print("Learning Move:", moveNamesList[newMove + 1])
    end
  end
end
